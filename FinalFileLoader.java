...
//static List<List<String>> FinalvalSet1=new ArrayList<List<String>>();
static int totalarg;
...
try (BufferedReader br = new BufferedReader(new FileReader(args[0]))) //Reading CSV File
		...
		for(int total=0;total<information.length;total++)
		{
			if(total>1)
			{
				mixitems=mixitems+information[total].trim(); //Concating the available product
				//valSet1.add(information[total].trim());
			}
			else
			{
...
int count2=0;
	...
	{
		mixcont=""; //Clearing data of inner list for further search
		mixcont=FinalvalSet.get(outter).get(2);
		int count=0;
		for(int srh=1;srh<totalarg;srh++) //Checking the products given through commnad line
		{	
			if(mixcont.contains(items[srh]))
			{
				count++; //Incrementing counter if product is found
				//int i=(Integer.parseInt(FinalvalSet.get(outter).get(0))-1);
...
else
{
	if(gotitems1==1) //Checking if the product is available
		{
System.out.println(" id1 "+id1+"  p "+price1+" c "+c1);
			minimum(id1,price1);// Finding minimum price from all the available hotels through iterator
			
		}
	else // Checking if product is available in combo or distributed manner
	{
